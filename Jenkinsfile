pipeline {
    ...
    stages {
        ...
        stage('Generate Environment File') {
            steps {
                // Utilisation de guillemets simples Groovy (''') pour empecher TOUTE interpolation Groovy
                // Les variables Jenkins sont injectees via env.VARIABLE dans le shell
                sh '''
                    set -e
                    rm -f .env
                    ...
                    "SMTP_FROM=${SMTP_FROM}" \
                    "ALERT_EMAIL_TO=${ALERT_EMAIL_TO}" > .env
